...
const Request = () => {
  ...
  // Transaction type configurations - Light Theme
  ...
  // Status filter configurations - Light Theme
  const statusConfig = {
    all: {
      label: "All",
      icon: Package,
  ...
  // Get overall approval status for display - SUPERVISOR VIEW
  const getApprovalStatus = (approval, transactionStatus) => {
    ...
    if (transactionStatus === 'pending') {
      ...
      // Check if supervisor has rejected
      ...
      // Supervisor hasn't taken action - awaiting your approval
      return { 
  ...
  // Fetch transactions from API - Updated for new response structure
  const fetchTransactions = useCallback(async (status = 'all', page = 1) => {
    try {
      ...
      // Set transactions from response
  ...
  // Calculate awaiting supervisor approval count
  const getAwaitingSupervisorApproval = () => {
    return transactions.filter(t => {
  ...
  // Calculate supervisor approved count (waiting for next level)
  const getSupervisorApprovedCount = () => {
    return transactions.filter(t => {
      if (t.transaction_status !== 'pending') return false;
      const supervisorApproval = getIndividualApprovalStatus(t.approval?.supervisor_approval);
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-teal-50/30 flex items-center justify-center p-4">
        <div className="text-center">
          <div className="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto">
            ...
            <div className="absolute inset-3 sm:inset-4 border-4 border-gray-100 rounded-full"></div>
            <div className="absolute inset-3 sm:inset-4 border-4 border-cyan-400 rounded-full border-b-transparent animate-spin" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }}></div>
            <UserCheck className="absolute inset-0 m-auto w-8 h-8 text-teal-500 animate-pulse" />
          </div>
          <p className="mt-6 sm:mt-8 text-gray-800 font-semibold text-base sm:text-lg">Loading supervisor dashboard...</p>
          <p className="mt-2 text-gray-500 text-xs sm:text-sm">Please wait while we fetch your data</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen ">
      <div className="max-w-7xl mx-auto  space-y-4 sm:space-y-6">
        {/* Header Section - Light Professional Theme for Supervisor */}
        <div className="relative bg-white rounded-2xl sm:rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
          {/* Decorative Background Elements */}
          <div className="absolute inset-0 bg-gradient-to-r from-teal-50/50 via-transparent to-cyan-50/50"></div>
          <div className="absolute top-0 right-0 w-64 sm:w-96 h-64 sm:h-96 bg-gradient-to-bl from-teal-100/40 to-transparent rounded-full -mr-32 sm:-mr-48 -mt-32 sm:-mt-48"></div>
          <div className="absolute bottom-0 left-0 w-48 sm:w-64 h-48 sm:h-64 bg-gradient-to-tr from-cyan-100/40 to-transparent rounded-full -ml-24 sm:-ml-32 -mb-24 sm:-mb-32"></div>
            ...
            <div className="flex flex-col gap-4 sm:gap-6">
              ...
              <div className="space-y-2 sm:space-y-3">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                  <span className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1 sm:py-1.5 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-full text-xs sm:text-sm font-bold shadow-lg shadow-teal-200">
                    <UserCheck className="w-3 h-3 sm:w-4 sm:h-4" />
                    Supervisor Panel
                  </span>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-semibold border border-gray-200">
                    <ClipboardCheck className="w-3 h-3 text-teal-500" />
                    Initial Approval
                  </span>
                  {counts.pending.count > 0 && (
                    <span className="px-2 sm:px-3 py-0.5 sm:py-1 bg-amber-50 text-amber-700 rounded-full text-xs font-bold flex items-center gap-1 border border-amber-200 animate-pulse">
                ...
                <p className="text-gray-600 text-sm sm:text-base md:text-lg max-w-xl">
                  Review and provide initial approval for farmer transactions before manager review.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Search by name, phone, warehouse..."
                    value={searchValue}
                    onChange={(e) => setSearchValue(e.target.value)}
                    className="w-full pl-10 sm:pl-12 pr-4 py-3 sm:py-3.5 bg-gray-50 border border-gray-200 rounded-xl sm:rounded-2xl focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400 focus:bg-white transition-all duration-300 placeholder:text-gray-400 text-gray-800 text-sm sm:text-base shadow-sm"
                  />
                  {searchValue && (
                    <button
                ...
                <button
                  ...
                  disabled={refreshing}
                  className={`
                    p-3 sm:p-3.5 bg-gray-100 border border-gray-200 rounded-xl sm:rounded-2xl 
                    hover:bg-gray-200 hover:border-gray-300 transition-all duration-300 self-end sm:self-auto shadow-sm
        ...
        {/* Transaction Type Tabs - Light Theme */}
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg shadow-gray-100 border border-gray-100 p-1.5 sm:p-2">
            ...
            {TransactionTypes.map(type => {
              ...
              return (
                <button
                  ...
                  className={`
                    relative p-3 sm:p-4 md:p-5 rounded-xl sm:rounded-2xl transition-all duration-300 group overflow-hidden
                    ${isActive 
                ...
                >
                  <div className="relative flex items-center gap-2 sm:gap-3">
                    ...
                    <div className="text-left min-w-0">
                      <p className={`font-semibold text-sm sm:text-base md:text-lg truncate ${isActive ? 'text-white' : 'text-gray-800'}`}>
                        {config.label}
                      </p>
                  ...
                  {pendingCount > 0 && type === 'deposit' && (
                    <div className="absolute -top-1 -right-1 min-w-[22px] sm:min-w-[28px] h-5 sm:h-7 px-1.5 sm:px-2 bg-red-500 text-white rounded-full flex items-center justify-center text-xs font-bold shadow-lg animate-pulse">
                      {pendingCount}
                    </div>
                  )}
                </button>
        ...
        {/* Statistics Cards - Light Professional Theme for Supervisor */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3 md:gap-4">
          ...
          <StatCard
            title="Your Action"
            value={getAwaitingSupervisorApproval()}
            icon={<AlertCircle className="w-4 h-4 sm:w-5 sm:h-5" />}
            gradient="from-amber-500 to-orange-500"
            bgColor="bg-amber-50"
            borderColor="border-amber-200"
            highlight={getAwaitingSupervisorApproval() > 0}
          />
          <StatCard
            title="Pending"
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl shadow-gray-100 border border-gray-100 overflow-hidden">
          {/* Status Filter Tabs */}
          <div className="border-b border-gray-100 bg-gray-50/50 px-3 sm:px-4 md:px-6 py-3 sm:py-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                    ...
                    <button
                      ...
                      className={`
                        ...
                        ${isActive 
                          ...
                          : `${config.textColor} ${config.bgHover} bg-white border border-gray-200 hover:shadow-sm`
                        }
              ...
              {activeTab !== "deposit" && (
                <span className="text-xs sm:text-sm text-gray-500 italic bg-gray-100 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg border border-gray-200">
              ...
              <div className="flex items-center justify-center py-12 sm:py-16">
                <div className="flex flex-col items-center gap-3 sm:gap-4">
                  <div className="relative">
                    <div className="animate-spin rounded-full h-10 w-10 sm:h-12 sm:w-12 border-4 border-gray-200 border-t-teal-500"></div>
                  </div>
                  <span className="text-gray-600 font-medium text-sm sm:text-base">Loading transactions...</span>
                </div>
              </div>
          ...
          {pagination.total_items > 0 && !searchValue && (
            <div className="border-t border-gray-100 px-3 sm:px-4 md:px-6 py-3 sm:py-4 bg-gray-50/50">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
                ...
                <div className="text-xs sm:text-sm text-gray-600 order-2 sm:order-1">
                  ...
                  of{" "}
                  <span className="font-semibold text-gray-800">{pagination.total_items}</span>
                </div>
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${currentPage === 1
                        ...
                        : 'border-gray-200 text-gray-600 hover:bg-teal-50 hover:border-teal-300 hover:text-teal-600'
                      }
                    `}
                    title="First Page"
                  >
                      ...
                      if (startPage > 1) {
                        pages.push(
                          <button
                            key={1}
                            onClick={() => goToPage(1)}
                            className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-medium text-gray-600 hover:bg-teal-50 hover:text-teal-600 transition-all"
                          >
                            1
                          </button>
                        );
                      ...
                      for (let i = startPage; i <= endPage; i++) {
                        pages.push(
                          <button
                            ...
                            onClick={() => goToPage(i)}
                            className={`
                              px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all duration-200
                              ${currentPage === i
...
// Stat Card Component - Light Professional Theme
const StatCard = ({ title, value, subValue, icon, gradient, bgColor, borderColor, isAmount = false, highlight = false }) => (
  <div className={`
    relative ${bgColor} rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-5 
    border ${borderColor} overflow-hidden group hover:shadow-lg 
    transition-all duration-300 hover:-translate-y-0.5
    ${highlight ? 'ring-2 ring-amber-400 ring-offset-2' : ''}
  `}>
    <div className="absolute top-0 right-0 w-16 sm:w-20 h-16 sm:h-20 bg-gradient-to-br from-white/80 to-transparent rounded-full -mr-8 sm:-mr-10 -mt-8 sm:-mt-10 group-hover:scale-150 transition-transform duration-500"></div>
    <div className="relative">
      <div className="flex items-start justify-between mb-1.5 sm:mb-2">
        <div className={`p-1.5 sm:p-2 md:p-2.5 bg-gradient-to-br ${gradient} rounded-lg sm:rounded-xl text-white shadow-md group-hover:scale-110 transition-transform duration-300`}>
          {icon}
        </div>
        {highlight && (
      ...
      <p className={`${isAmount ? 'text-sm sm:text-lg md:text-xl' : 'text-xl sm:text-2xl md:text-3xl'} font-bold text-gray-800 truncate`}>
        {value}
      </p>
      {subValue && (
    ...
    switch (statusFilter) {
      case 'pending':
        return {
          ...
          subtitle: 'All transactions have been reviewed. Great job!'
        };
      case 'completed':
        return {
      ...
      default:
        return {
          ...
          subtitle: 'New farmer requests will appear here for your initial review'
        };
    }
  };
  ...
  return (
    <div className="text-center py-12 sm:py-16 md:py-24">
      <div className={`
        ...
        bg-gray-100 rounded-2xl sm:rounded-3xl mb-4 sm:mb-6 shadow-inner
      `}>
        <Icon className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-gray-400`} />
      </div>
      <p className="text-gray-800 text-lg sm:text-xl md:text-2xl font-bold mb-1.5 sm:mb-2">
        {message.title}
      </p>
      ...
      {searchValue && (
        <button
          ...
          className="mt-4 sm:mt-6 px-5 sm:px-6 py-2 sm:py-2.5 bg-teal-500 hover:bg-teal-600 text-white rounded-lg sm:rounded-xl font-medium shadow-md hover:shadow-lg transition-all text-sm sm:text-base active:scale-95"
        >
          Clear Search
        </button>
      )}
...
// Transaction Card Component - Supervisor Light Professional Theme
const TransactionCard = ({ transaction, config, onApprove, onReject, onView, getApprovalStatus, getIndividualApprovalStatus, index }) => {
  ...
  // SUPERVISOR CAN APPROVE when transaction is pending and supervisor hasn't taken action
  const canSupervisorApprove = isPending && supervisorApproval.status === 'pending';

  // Check if supervisor already approved
  const supervisorHasApproved = supervisorApproval.status === 'approved';

  return (
    <div 
      className={`
        group bg-white rounded-xl sm:rounded-2xl border transition-all duration-300 overflow-hidden
        ${canSupervisorApprove 
          ? 'border-amber-300 hover:border-amber-400 shadow-lg shadow-amber-100/50' 
          : supervisorHasApproved && isPending
          ? 'border-teal-200 hover:border-teal-300 shadow-md shadow-teal-50'
          : isRejected 
    ...
    >
      {/* Action Required Banner for Supervisor */}
      {canSupervisorApprove && (
        <div className="bg-gradient-to-r from-amber-400 to-orange-400 px-3 sm:px-4 py-2 sm:py-2.5">
          <div className="flex items-center gap-2 text-amber-900">
            <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5" />
            <span className="text-xs sm:text-sm font-semibold">New Request - Awaiting Your Initial Approval</span>
          </div>
        </div>
      )}
      ...
      {supervisorHasApproved && isPending && (
        <div className="bg-gradient-to-r from-teal-400 to-cyan-400 px-3 sm:px-4 py-2 sm:py-2.5">
          <div className="flex items-center gap-2 text-teal-900">
            <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5" />
            <span className="text-xs sm:text-sm font-semibold">
              You Approved - Awaiting {managerApproval.status === 'approved' ? 'Admin' : 'Manager'} Approval
            </span>
          </div>
        </div>
      )}
      ...
      <div className="p-3 sm:p-4 md:p-6">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-start gap-3 sm:gap-4">
            {/* Icon */}
            <div className={`
              p-2.5 sm:p-3 md:p-4 rounded-xl sm:rounded-2xl border shrink-0
              ${canSupervisorApprove
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2 sm:mb-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 sm:py-1 bg-gray-100 text-gray-600 rounded-md sm:rounded-lg text-[10px] sm:text-xs font-medium capitalize border border-gray-200">
                    {transaction.user?.role || 'N/A'}
                  </span>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                  <span className={`
                    inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-semibold border
                    ${approvalStatus.color}
                  `}>
                    <span className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${approvalStatus.dotColor} ${canSupervisorApprove ? 'animate-pulse' : ''}`}></span>
                    <span className="hidden xs:inline">{approvalStatus.text}</span>
                  </span>
                  <span className="text-[10px] sm:text-xs text-gray-400 font-mono bg-gray-50 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border border-gray-200">
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-3 text-xs sm:text-sm text-gray-500">
                {transaction.user?.phone_number && (
                  <div className="flex items-center gap-1 bg-gray-50 px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg border border-gray-100">
              ...
              {firstGrain.category && (
                <div className="flex flex-wrap items-center gap-2 mb-3 sm:mb-4">
                  <span className={`
                    px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold border
                    ${isRejected 
              ...
              {/* Remarks */}
              {transaction.remarks && (
                <div className="px-3 sm:px-4 py-2 sm:py-2.5 bg-gray-50 rounded-lg sm:rounded-xl mb-3 sm:mb-4 border border-gray-100">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 pt-3 sm:pt-4 border-t border-gray-100">
            ...
            {/* Action Buttons - Supervisor Version */}
            <div className="flex items-center gap-2 w-full sm:w-auto">
              {canSupervisorApprove && (
                <>
                  <button
                    onClick={onApprove}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg sm:rounded-xl transition-all duration-200 hover:shadow-lg active:scale-95 font-semibold text-xs sm:text-sm"
                  >
                    <CheckCircle className="w-4 h-4 sm:w-5 sm:h-5" />
                    <span>Approve</span>
                  </button>
                  <button
                    onClick={onReject}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-lg sm:rounded-xl transition-all duration-200 hover:shadow-lg active:scale-95 font-semibold text-xs sm:text-sm"
                  >
              ...
              <button
                onClick={onView}
                className={`
                  ${canSupervisorApprove ? '' : 'flex-1 sm:flex-none'} 
                  ...
                  transition-all duration-200 hover:shadow-md active:scale-95 font-semibold text-xs sm:text-sm border border-gray-200
                `}
              >
...
// Approval Badge Component - Light Theme for Supervisor
const ApprovalBadge = ({ label, approvalData, isCurrentUser = false }) => {
  const getStatusStyles = () => {
    switch (approvalData.status) {
      case 'approved':
        return {
          bg: isCurrentUser ? 'bg-teal-50' : 'bg-emerald-50',
          border: isCurrentUser ? 'border-teal-200' : 'border-emerald-200',
          text: isCurrentUser ? 'text-teal-700' : 'text-emerald-700',
      ...
      default:
        return {
          bg: isCurrentUser ? 'bg-amber-50' : 'bg-gray-50',
  ...
  return (
    <div className={`
      flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-medium
      ${styles.bg} ${styles.text} border ${styles.border}
      ${isCurrentUser && approvalData.status === 'pending' ? 'ring-2 ring-amber-300 ring-offset-1 shadow-sm' : ''}
      transition-all duration-200
    `}>
      {isCurrentUser && approvalData.status === 'pending' && <UserCheck className="w-3 h-3 text-amber-600" />}
      {styles.icon}
      <span className="hidden xs:inline">{label}</span>
      <span className="xs:hidden">{label.slice(0, 3)}</span>
      {isCurrentUser && <span className="text-amber-600 text-[9px]">(You)</span>}
      {approvalData.hasAction && approvalData.user && (
        <span className="text-gray-500 text-[9px] ml-0.5">
          • {isCurrentUser && approvalData.status === 'approved' ? 'You' : approvalData.user.name?.split(' ')[0]}
        </span>
      )}
    </div>
  );
...
// Info Badge Component - Light Professional Theme
const InfoBadge = ({ icon, label, color = "gray" }) => {
  ...
  return (
    <div className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 sm:py-2 rounded-lg border ${colorStyles[color]} transition-all hover:shadow-sm`}>
